...
class MapReaderRandomGameScoring(val taskObjects:ArrayBuffer[FastObject], box:Box) extends Scorer {
...
class MapReaderRandomGame(val locations:Array[Room], val taskObjects:ArrayBuffer[FastObject], val mapbook:Mapbook, val box:Box, val startLocation:Room, val endLocation:Room, val actualDistanceApart:Int, val limitInventorySize:Boolean, val seed:Long = 0, val generationProperties:Map[String, Int]) extends TextGame {
  ...
  def deepCopy():MapReaderRandomGame = {

    // Return
    new MapReaderRandomGame(locations, taskObjects, mapbook, box, startLocation, endLocation, actualDistanceApart, limitInventorySize, seed, generationProperties)
  }
...
class MapReaderRandomGameGenerator {
  ...
  def mkGame(seed:Long, numLocations:Int = 15, maxDistanceApart:Int = 1, numDistractorItems:Int = 0, includeDoors:Boolean = false, limitInventorySize:Boolean = false, fold:String = "train"):MapReaderRandomGame = {
  ...
  def mkGameWithGoldPath(seed:Long, numLocations:Int = 15, maxDistanceApart:Int = 1, numDistractorItems:Int = 0, includeDoors:Boolean = false, limitInventorySize:Boolean = false, fold:String = "train"):(MapReaderRandomGame, Array[String]) = {
