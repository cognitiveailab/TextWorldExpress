...
class SimonSaysGame(val goldActionSequence:Array[String], val possibleActions:Array[String], val memorization:Boolean, val seed:Long = 0, val generationProperties:Map[String, Int]) extends TextGame {
  ...
  def deepCopy():SimonSaysGame = {
    ...
    // Return
    return new SimonSaysGame(goldActionSequence, possibleActions, memorization, seed, generationProperties)
  }
  ...
  def mkObservation(curStage:Int):String = {
    ...
    if (this.memorization) {
      ...
      return "Simon says, take this action: " + this.goldActionSequence(curStage)
    }
  }
...
class SimonSaysGameGenerator {
  ...
  def mkGame(seed:Long, fold:String = "train", gameLength:Int = 0, numDistractors:Int = 0, memorization:Boolean = false):SimonSaysGame = {
    ...
    props("memorization") = if (memorization) { 1 } else { 0 }

    // Generate Game
    val actionSequence = this.mkActionSequence(r, gameFold = fold, length = gameLength+numDistractors)  // Overgenerate by numDistractors, to have distractors
    val goldSequence = actionSequence.slice(0, gameLength)
    val possibleActions = actionSequence

    val game = new SimonSaysGame(goldSequence, possibleActions, memorization, seed, generationProperties = props.toMap)

    return game
  }


  def mkGameWithGoldPath(seed:Long, fold:String = "train", gameLength:Int = 0, numDistractors:Int = 0, memorization:Boolean = false):(SimonSaysGame, Array[String]) = {
    ...
    breakable {
      while (attempts < MAX_ATTEMPTS) {
        val game = this.mkGame(seed, fold, gameLength, numDistractors, memorization)
